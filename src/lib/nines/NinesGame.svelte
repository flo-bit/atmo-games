...
<div class="flex w-full flex-col items-center gap-4 px-2 py-4 sm:px-4">
	<div class="flex w-full max-w-md items-center justify-between px-1">
		<div class="flex items-center gap-3">
			<span class="rounded-full bg-base-200 px-3 py-1 text-xs font-bold uppercase tracking-wider text-base-600 dark:bg-base-800 dark:text-base-400">
				{puzzle.difficulty}
			</span>
		</div>
		<div class="flex items-center gap-3">
			<div class="flex items-center gap-1 text-sm">
				{#each Array(3) as _, i (i)}
					<span class="{i < 3 - game.mistakes ? 'opacity-100' : 'opacity-25'} text-red-500">&hearts;</span>
				{/each}
			</div>
			<span class="font-mono text-sm tabular-nums text-base-600 dark:text-base-400">{game.formattedTime}</span>
		</div>
	</div>
	...
	<div class="sudoku-grid aspect-square w-full max-w-md" role="grid">
		{#each Array(9) as _, row (row)}
			{#each Array(9) as _, col (col)}
				...
				<button
					class="sudoku-cell flex items-center justify-center border-base-300 dark:border-base-600
						{game.cellClasses(row, col)}
						{isInitial ? 'font-extrabold' : 'font-semibold'}
						{col % 3 === 0 && col !== 0 ? 'border-l-2 border-l-base-500 dark:border-l-base-400' : 'border-l'}
						{row % 3 === 0 && row !== 0 ? 'border-t-2 border-t-base-500 dark:border-t-base-400' : 'border-t'}
						{col === 8 ? '' : ''}
						{row === 8 ? '' : ''}"
					...
					{:else if cellNotes.size > 0}
						<div class="notes-grid">
							{#each [1, 2, 3, 4, 5, 6, 7, 8, 9] as n (n)}
								<span class="text-[8px] leading-none sm:text-[10px] {cellNotes.has(n) ? 'text-base-500 dark:text-base-400' : 'text-transparent'}">{n}</span>
							{/each}
						</div>
					{/if}
				</button>
	...
	{#if game.gameState === 'playing'}
		...
		<div class="grid w-full max-w-md grid-cols-9 gap-1">
			{#each [1, 2, 3, 4, 5, 6, 7, 8, 9] as num (num)}
				<button
					class="flex aspect-square cursor-pointer items-center justify-center rounded-lg bg-base-200 text-lg font-bold text-base-800 transition-colors hover:bg-base-300 active:bg-base-400 dark:bg-base-800 dark:text-base-200 dark:hover:bg-base-700 dark:active:bg-base-600 sm:text-xl"
	...
	{:else}
		<div class="flex flex-col items-center gap-2 py-4">
			...
			<span class="text-sm text-base-500 dark:text-base-400">Too many mistakes</span>
		</div>
	{/if}
</div>
