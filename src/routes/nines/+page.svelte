...
<div class="flex min-h-svh flex-col items-center px-4 py-8">
	<div class="mb-4 flex flex-col items-center gap-2">
		<a href="/" class="text-sm text-base-400 hover:text-base-600 dark:text-base-500 dark:hover:text-base-300">&larr; atmo games</a>
		<h1 class="text-2xl font-bold text-base-800 dark:text-base-200">Nines</h1>
		<p class="text-sm text-base-500 dark:text-base-400">Fill every row, column, and box with 1-9.</p>
	</div>

	<div class="mb-4 flex flex-wrap justify-center gap-2">
		{#each ['easy', 'medium', 'hard', 'expert'] as diff (diff)}
			<button
				class="cursor-pointer rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider transition-colors
					{difficulty === diff
	...
	<div class="w-full max-w-lg">
		{#key gameKey}
			<NinesGame {puzzle} onGameEnd={(won, time) => {
